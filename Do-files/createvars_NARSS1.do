...
//Create the sanitation indicators and covariates from the NARSS1 data 

///// Access is defined at Household Level 

cd "$data" 

use "NARSSHH1_With_District.dta", clear

rename _merge merge_dist

duplicates drop UID, force

gen cluster=VillName
...
replace IMPROVED_LATRINE_2=0 if IMPROVED_LATRINE_1==.
...
save analysis_sanitation_narss1_hh, replace

*using individual level data set to make assumptions about use
	
use NARSSIND1.dta, clear
...
save analysis_sanitation_narss1, replace

use analysis_sanitation_narss1.dta, clear
collapse(mean) USAGE_ALWAYS, by(UID)
ren USAGE_ALWAYS USAGE_ALWAYSHH
sort UID
save temp2, replace

use analysis_sanitation_narss1.dta
...
merge 1:1 UID using analysis_sanitation_narss1_hh.dta
...
save Analysis_Sanitation_NARSS1.dta, replace
