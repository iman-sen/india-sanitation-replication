//Create the sanitation indicators and covariates from the NARSS2 data 
...
use NARSSHH2_With_District.dta, clear
...
gen OWN_HH_TOILET=1 if Q1_HH=="Yes- We have access to toilet Exclusively used by our family"
replace OWN_HH_TOILET=0 if OWN_HH_TOILET==.

//// Access to own or shared toilet

gen HH_TOILET=1 if Q1_HH=="Yes- We have access to toilet Exclusively used by our family" | Q1_HH=="Yes- We have access to toilet used by multiple families"
replace HH_TOILET=0 if HH_TOILET==.

/// Access to shared toilet
gen SHARED_TOILET=1 if Q1_HH=="Yes- We have access to toilet used by multiple families"
...
replace IMPROVED_LATRINE_1=1 if  Q4_HH=="Septic tank with a soak Pit"
...
replace IMPROVED_LATRINE_2=1 if  Q4_HH=="Septic tank with a soak Pit"
replace IMPROVED_LATRINE_2=1 if  Q4_HH=="Septic tank without soak pit"
replace IMPROVED_LATRINE_2=1 if  Q4_HH=="Open pit"
replace IMPROVED_LATRINE_2=1 if  Q4_HH==" "
replace IMPROVED_LATRINE_2=0 if IMPROVED_LATRINE_2==.

///unimproved sanitation
gen UNIMPROVED_LATRINE=1 if Q4_HH=="Septic tank without soak pit"
replace UNIMPROVED_LATRINE=1 if Q4_HH=="Open pit"
replace UNIMPROVED_LATRINE=1 if Q4_HH==" "

save analysis_sanitation_narss2_hh, replace

*using individual level data set to make assumptions about use
	
use NARSSIND2.dta, clear
...
gen USAGE_ALWAYS=1 if  Q7D_HH_=="Yes"
replace USAGE_ALWAYS=0 if USAGE_ALWAYS==.
save analysis_sanitation_narss2, replace

use analysis_sanitation_narss2.dta, clear
collapse(mean) USAGE_ALWAYS, by(UID)
ren USAGE_ALWAYS USAGE_ALWAYSHH
sort UID
save temp2, replace

use analysis_sanitation_narss2.dta
...
merge 1:1 UID using analysis_sanitation_narss2_hh.dta
...
gen weight_hh=RevisedWeights
gen weight_ind = RevisedWeights*hhsize

gen soc_grp = ""
replace soc_grp = "SC" if D3_HH == "Scheduled Caste"
replace soc_grp = "ST" if D3_HH == "Scheduled Tribe"
replace soc_grp = "OBC" if D3_HH == "Other Backward Caste"
replace soc_grp = "GEN" if (D3_HH == "General Caste" | D3_HH == "Don't Know/Can't Say")
...
save Analysis_Sanitation_NARSS2.dta, replace
